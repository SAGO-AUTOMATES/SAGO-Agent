...
> **Production-grade multi-agent orchestration system** — 339 specialist agents, 45 tools, multi-LLM support, streaming, workflows, TUI, and built-in security.
...
| **TUI Interface** | Rich terminal UI with autocomplete, collapsible tool calls, and command history |
...
├── tui/                 # Terminal UI
│   └── app.py           # Textual TUI with autocomplete
├── llm/                 # LLM providers
│   ├── openai_provider.py
│   ├── openrouter.py
│   ├── gemini.py
...
# Run with coverage
pytest tests/ --cov=sago --cov-report=html
```
...
|----------|-------|----------|
| Unit - Tools | 30 | All 45 tools tested with proper args |
| Unit - Permissions | 14 | Risk levels, blocking, approval workflow |
| Unit - Agents | 13 | Registry, profiles, lookup |
| Integration - Executor | 10 | Tool discovery, task detection, extraction |
| Integration - Server | 6 | Daemon, client, protocol |
| Integration - Workflow | 9 | Engine, steps, dependencies |
| Integration - MCP | 7 | Server, tools, creation |
