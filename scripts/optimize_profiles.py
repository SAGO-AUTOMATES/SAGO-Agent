...
def optimize_agent_profile(filepath: Path) -> bool:
    ...
    new_content = content.replace(
        f'system_prompt="""{original_prompt}"""', f'system_prompt="""{optimized_prompt}"""'
    )
...
def main():
    ...
    print(
        f"Saved {total_original_lines - total_optimized_lines} lines ({((total_original_lines - total_optimized_lines) / total_original_lines * 100):.1f}%)"
    )
